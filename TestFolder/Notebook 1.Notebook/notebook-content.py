...
# Databricks notebook: Raw to Silver - SalesOrderDetail Incremental
from pyspark.sql.functions import col, to_date, lit, when, current_date, current_timestamp, date_sub
...
# 6. Filter only last 14 days for overwrite
df_last14 = df_silver.filter(
    col("ModifiedDate") >= date_sub(current_date(), 14)
)

# 7. Overwrite only those dates in curated layer
df_last14.write \
    .format("delta") \
    .mode("overwrite") \
